...
//add employee funct
const getRoles = async (): Promise<{ title: string; id: number }[]> => {
    try {
...
const getEmployees = async (): Promise<{ name: string; id: number }[]> => {
    try {
...
const addRole = async() => {
    ...
    const getDepartments = async (): Promise<{ name: string; id: number }[]> => {
        try {
    ...
    const {roleName, salary} = await inquirer.prompt([
        ...
        },
        {
            ...
            validate: (input: string) => isNaN(Number(input)) ? 'Debe ser un número válido.' : true,
